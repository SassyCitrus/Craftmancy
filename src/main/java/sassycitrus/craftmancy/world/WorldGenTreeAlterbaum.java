...
/*
    References WorldGenWeedwoodTree.java for tree generation
    From https://github.com/Angry-Pixel/The-Betweenlands.
*/
public class WorldGenTreeAlterbaum extends WorldGenAbstractTree
{
    private static final IBlockState LOGX = CraftmancyBlocks.ALTERBAUM_LOG.getDefaultState()
            .withProperty(BlockLog.LOG_AXIS, BlockLog.EnumAxis.X);

    private static final IBlockState LOGY = CraftmancyBlocks.ALTERBAUM_LOG.getDefaultState()
            .withProperty(BlockLog.LOG_AXIS, BlockLog.EnumAxis.Y);

    private static final IBlockState LOGZ = CraftmancyBlocks.ALTERBAUM_LOG.getDefaultState()
    ...
    private static final IBlockState WOOD = CraftmancyBlocks.ALTERBAUM_WOOD.getDefaultState();
        ...
        for (int x1 = -maxRadius; x1 <= maxRadius; x1++)
        {
            for (int z1 = -maxRadius; z1 <= maxRadius; z1++)
            {
                for (int y1 = 0; y1 < maxRadius; y1++)
                {
                    ...
                    {
                        if (!isLog(world.getBlockState(pos.add(x1, y1, z1))))
                        {
        ...
        for (int x1 = -maxRadius; x1 <= maxRadius; x1++)
        {
            for (int z1 = -maxRadius; z1 <= maxRadius; z1++)
            {
                for (int y1 = 0; y1 < maxRadius; y1++)
                {
                    ...
                    {
                        if (!isLog(world.getBlockState(pos.add(x1, y1, z1))) && rand.nextInt(5) != 0)
                        {
                    ...
                    if (Math.round(Math.sqrt(dSq)) <= maxRadius && rand.nextInt(3) == 0 && y1 == 0)
                    {
                        if (world.getBlockState(pos.add(x1, y1, z1)) == LEAVES)
                        {
                            ...
                            {
                                if (!isLog(world.getBlockState(pos.add(x1, y1 - i, z1))))
                                {
    ...
    {
        return state.getBlock() instanceof LogAlterbaum;
    }
